...
export function TaskList() {
  ...
  return (
    <div className={cn("flex flex-col", isMobile ? 'h-[90vh]' : 'flex-1')}>
      {/** タスク一覧コンテンツ */}
      <Card className='h-full flex flex-col overflow-hidden'>
        <CardHeader>
          <CardTitle className={cn("flex gap-2", isMobile ? 'flex-col' : 'justify-between items-center')}>
              ...
              <Sheet open={isFilterOpen} onOpenChange={setIsFilterOpen}>
                <SheetTrigger asChild>
                  <Button variant="outline" size="sm">
                    <Filter className={cn("h-3 w-3", !isMobile && 'mr-2')} />
                    {!isMobile && 'フィルター'}
                  </Button>
                </SheetTrigger>
                <SheetContent>
                      ...
                      return (
                        <TableRow key={task.uid}>
                          <TableCell>
                            ...
                            >
                              <SelectTrigger className={cn("w-28", getStatusColor(task.status))}>
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
                                <SelectItem value="not_started">未着手</SelectItem>
